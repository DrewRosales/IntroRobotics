...
class QuadMap:
    ...
    def ray_update(self, origin: np.ndarray, endpoint: np.ndarray):
        """
        This function updates the map based upon the return of a rangefinding beam.
    ...
    def traverse(self, node, depth, max_depth, grid):
        if not node.children:
            ...
            return
        for child in node.children:
            self.traverse(child, depth + 1, max_depth, grid)
    ...
    def to_occupancygrid(self):
        ...
        """
        grid = []
        self.traverse(self.root, 0, self.max_depth, grid)
        return grid
